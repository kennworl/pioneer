...
local function button_wheelstate()
	...
	if wheelstate == 0.0 then -- gear is up
		ui.sameLine()
		if mainMenuButton(icons.landing_gear_down, false, lui.HUD_BUTTON_LANDING_GEAR_IS_UP) or (ui.noModifierHeld() and ui.isKeyReleased(ui.keys.f6)) or SerialComms.isLandingGearButtonReleased() then
			player:ToggleWheelState()
		end
		SerialComms.WriteData("lgu~")
	elseif wheelstate == 1.0 then -- gear is down
		ui.sameLine()
		if mainMenuButton(icons.landing_gear_up, false, lui.HUD_BUTTON_LANDING_GEAR_IS_DOWN) or (ui.noModifierHeld() and ui.isKeyReleased(ui.keys.f6)) or SerialComms.isLandingGearButtonReleased() then
			player:ToggleWheelState()
		end
